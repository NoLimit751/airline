...
object LoanInterestRateSimulation {
  ...
  val PREVIOUS_RATE_ENTRIES_TO_CONSIDER = 50

  def simulate(cycle: Int) : Unit = {
    val fromCycle = cycle - PREVIOUS_RATE_ENTRIES_TO_CONSIDER //last 50 weeks have influence on the rate calculation (changed from 20)
  ...
  val DEFAULT_ANNUAL_RATE : Double = 0.08 //8% (changed from 12%)
  val MAX_DELTA : BigDecimal = 0.01
  val RATE_STEP : BigDecimal= 0.001 // 0.1 % is a step
  val MIN_RATE : BigDecimal= 0.01 //min rate is 1%
  val MAX_RATE : BigDecimal= 0.08 //max rate is 8% annual (changed from 23%)
